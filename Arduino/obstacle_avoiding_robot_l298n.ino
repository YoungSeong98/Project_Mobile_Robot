...
int doPing () {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Detect a pulse on the Echo pin 8. pulseIn() measures the time in 
    // microseconds until the sound pulse returns back to the sensor    
    ...
    average += distance; // Compute running sum
  
    delay(10);           // Wait 10 milliseconds between pings
  }
